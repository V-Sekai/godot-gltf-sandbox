...
#		if extensions.has("MOZ_hubs_components"):
#			import_moz_hubs(gstate, path, node_i, nodes, nodes[node_i], extensions)
#		if extensions.has("import_material_unlit"):
#			import_material_unlit(gstate, path, node_i, nodes, nodes[node_i], extensions)

	return root_node


func import_omi_audio_emitter(gstate : GLTFState, json, node_3d, path, index, json_nodes, extensions : Dictionary):	
...
#
#func import_material_unlit(gstate, path, index, nodes, node, extensions):	
#	var node_3d : MeshInstance3D = gstate.get_scene_node(index)
...
#
#func import_moz_hubs(gstate, path, index, nodes, gltf_node, extensions):	
